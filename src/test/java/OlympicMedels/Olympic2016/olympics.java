package OlympicMedels.Olympic2016;
...
public class olympics {
	...
	public void most() throws InterruptedException {
		...
		for (int i = 0; i < rows.size(); i++) {
			...
			medalTable.add(new String[] { driver.findElement(By.xpath(tablePath + "tr[" + (i + 1) + "]//a")).getText(),
		...
		System.out.println(mostMedalCountry);
		Assert.assertEquals(mostMedalCountry, "United States"); // TOTAL 18
																// silver
		...
		// country row & col
		System.out.println(Arrays.toString(rowColNumber(medalTable, "Italy")));
	...
	@Test
	public void getSum() {
		Map<String, String>  countriesOfBronza = null;
		List<String> actual = getSumOfMedal( countriesOfBronza, 18);
		List<String> expected = Arrays.asList("Italy", "Australia");

		Assert.assertEquals(actual, expected);
	}

	public List<String> getSumOfMedal(Map<String, String>sumOfMedalsCountry , int sum) {
		List<String> sumOfMedal = new ArrayList<>();
		Set<Entry<String, String>> nSet = sumOfMedalsCountry.entrySet();
		List<Entry<String, String>> nList = new ArrayList<>(nSet);
		for (int i = 0; i < 10; i++) {
			for (int j = i + 1; j < 10; j++) {
				if (Integer.parseInt(nList.get(i).getValue())
						+ Integer.parseInt(nList.get(j).getValue()) == sum) {
					sumOfMedal.add(nList.get(i).getKey());
					sumOfMedal.add(nList.get(j).getKey());
				}
			}
		}
		return sumOfMedal;
